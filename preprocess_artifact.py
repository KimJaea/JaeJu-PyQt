...
if(len(sys.argv) == 3):
    ...
    file = "D:/ThinkGearData/sample/data.txt"
    path = "D:/ThinkGearData/sample"
...
f = open(path + "/file0.txt", "w")
for i in arr:
    f.write(str(int(i)) + " ")
f.close()

## txt 형태로 측정된 raw data 가져오기
f = open(path + "/file0.txt", "r")
con = f.readline()
f.close()
...
sio.savemat(path + "/data0.mat", data)
## xmax - 측정 시간에 맞춰 변경

EEG = octave.pop_loadset(path + "/data0.mat")
...
f = open(path + "/filted_data0.txt", 'w')
for i in con:
...
np.savetxt(path + "/parsed_data0.txt", y, delimiter = ',')

## 사용할 수 있는 실수 형태로 변환
f = open(path + "/parsed_data0.txt", "r")
con = f.readlines()
f.close()
